...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        key_position_ids = torch.arange(kv_seq_len, dtype=torch.long, device=position_ids.device).unsqueeze(0).view(-1,
                                                                                                                    kv_seq_len)
        ...
        if self.log_scale:
            log_n = torch.log(torch.tensor(kv_seq_len * 1.0)).to(query_states.device, dtype=query_states.dtype) / \
                    torch.log(torch.tensor(self.config.max_position_embeddings)).to(query_states.device,
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    _keep_in_fp32_modules = ["model.clex_layer.proj_func.ode_up_proj", "model.clex_layer.proj_func.ode_down_proj",
                             "model.clex_layer.inv_freq"]
    ...
    def _set_gradient_checkpointing(self, enable, gradient_checkpointing_func):
        if isinstance(gradient_checkpointing_func, LlamaModel):
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: CLEXLlamaConfig):
        ...
        if config.rope_scaling["type"] == "clex":
            self.clex_layer = LlamaCLEXScalingRotaryEmbedding(head_dim, config.max_position_embeddings,
                                                              config.rope_scaling)
